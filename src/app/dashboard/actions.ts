...
export interface ProjectActivity {
  id: string;
  projectId: string;
  type: 'project_created' | 'idea_added' | 'assumption_challenged' | 'experiment_generated' | 'status_changed' | 'project_updated';
  description: string;
  timestamp: string;
...
export async function createProject(input: CreateProjectInput): Promise<{ 
  ...
  console.log('[Server Action] createProject called:', { name: input.name, status: input.status });
...
export async function updateProject(input: UpdateProjectInput): Promise<{ 
  ...
  try {
    ...
    if (validatedInput.name !== undefined) updates.name = sanitizeText(validatedInput.name);
    if (validatedInput.description !== undefined) updates.description = sanitizeText(validatedInput.description);
    if (validatedInput.status !== undefined) updates.status = validatedInput.status;
    if (validatedInput.tags !== undefined) updates.tags = validatedInput.tags.map(tag => sanitizeText(tag));
...
export async function logProjectActivity(
  ...
  console.log('[Server Action] logProjectActivity called:', { projectId, type });
...
export async function linkItemToProject(
  ...
  console.log('[Server Action] linkItemToProject called:', { projectId, itemType, itemId });
  ...
  try {
    ...
    const activityType = 
      itemType === 'idea' ? 'idea_added' :
      itemType === 'assumption' ? 'assumption_challenged' :
      'experiment_generated';
    
    const description = 
      itemType === 'idea' ? `Added idea: "${itemTitle}"` :
      itemType === 'assumption' ? `Challenged assumption: "${itemTitle}"` :
    ...
    console.log('[Server Action] Linked item to project:', { projectId, itemType, itemId });
...
export async function calculateDashboardStats(
  ...
  try {
    ...
    const ideasThisWeek = ideas.filter(idea => {
      const ideaDate = new Date(idea.timestamp || idea.createdAt || 0);
...
export async function validateProjectName(
  ...
  try {
    ...
    const duplicate = existingProjects.find(p => 
      p.name.toLowerCase() === normalizedName && p.id !== excludeId
    );
